...
{
  m_downloader->DownloadAsString(SERVER_INDEX_FILE, true /* first pass */, [this](std::string const & buffer)
  {
    LOG(LDEBUG, (SERVER_INDEX_FILE, "downloaded"));
    ...
    m_downloader->DownloadAsString(downloader::GetFileDownloadUrl(COUNTRIES_FILE, dataVersion), false /* first pass */,
...
void Storage::ApplyCountries(std::string const & countriesBuffer, Storage & storage)
{
  ...
  if (!m_downloadingCountries.empty() || storage.m_currentVersion <= m_currentVersion)
    return;
  m_failedCountries.clear();
